...
func (div *CatmullClarkSubdivider) SubdivideOneMesh(mesh *AiMesh,
	out *AiMesh,
	num int,
	discard_input bool) error {
	if mesh == out {
		return errors.New("mesh == out")
	}
	return div.Subdivide([]*AiMesh{mesh}, &out, num, discard_input)
}
...
func (div *CatmullClarkSubdivider) Subdivide(smesh []*AiMesh,
	...
	num int,
	discard_input bool) {
	if smesh == nil || out == nil || smesh < out || smesh+nmesh > out+nmesh {
...
func (div *CatmullClarkSubdivider) InternSubdivide(smesh []*AiMesh,
	...
	spatial.Finalize()
	num_unique := spatial.GenerateMappingTable(maptbl, common.ComputePositionEpsilon(smesh, nmesh))
...
centroids:=make([]*Vertex,totfaces);
	...
	for ; t < len(smesh); t++ {
		...
		for  i := 0; i < len(mesh.Faces; i++ {
			...
			c=c.Div(c,float32(len(face.Indices)))
...
e := edges[MAKE_EDGE_HASH(mp[0], mp[1])];
e.ref++;
if (e.ref <= 2) {
...
break;
 ...
 GET_ADJACENT_FACES_AND_CNT:=func (vidx int ) ( fstartout *int , numout int ){
	...
	return
		...
		var new_points []common.Pair[bool, *Vertex]
// ---------------------------------------------------------------------
...
e0 := edges[MAKE_EDGE_HASH(maptbl[FLATTEN_VERTEX_IDX(t, int(face.Indices[a]))],
maptbl[FLATTEN_VERTEX_IDX(t, int(face.Indices[ tmp ]))])]; // fixme: replace with mod face.mNumIndices?
	tmp=a - 1
	if a==0{
		tmp=len(face.Indices) - 1
	}
// c) adjacent edge on the right, seen from the centroid
e1 := edges[MAKE_EDGE_HASH(maptbl[FLATTEN_VERTEX_IDX(t, int(face.Indices[a]))],
maptbl[FLATTEN_VERTEX_IDX(t, int(face.Indices[tmp]))])]; // fixme: replace with mod face.mNumIndices?
...
ai_assert(adj[o] - moffsets[nidx].first < len(mp.Faces));
const aiFace &f = mp.mFaces[adj[o] - moffsets[nidx].first];
haveit := false;

// find our original point in the face
for  m := 0; m < f.mNumIndices; m++ {
...
c0 := edges[MAKE_EDGE_HASH(org, maptbl[FLATTEN_VERTEX_IDX(nidx, f.mIndices[tmp])])];
// fixme: replace with mod face.mNumIndices?
	tmp= m + 1
	if m ==len(f.Indices) - 1{
		tmp=0
	}
c1 := edges[MAKE_EDGE_HASH(org, maptbl[FLATTEN_VERTEX_IDX(
nidx, f.Indices[  tmp])])];
// fixme: replace with mod face.mNumIndices?
...
div := float32(cnt)
divsq := 1. / (div * div);
ov.second = Vertex(minp, face.Indices[a]) * ((div - 3.) / div) + R * divsq + F * divsq;
}
}
	v++
